...
class Player(tk.Frame):
    ...
    def track_widgets(self):
        ...
        self.songtrack = tk.Label(self.track, font=("times new roman", 16, "bold"),
                                  bg="black", fg="red")
        self.songtrack['text'] = 'SUSHANT_PLAYER'
        self.songtrack.config(width=0, height=1)
        ...
        for root_, dirs, files in os.walk(directory):
            for file in files:
                if os.path.splitext(file)[1] == '.mp3':
                    path = os.path.join(root_, file).replace('\\', '/')
    ...
    def change_volume(self, event=None):
        volume_level = self.volume.get()
        mixer.music.set_volume(volume_level / 10)
...
if __name__ == "__main__":
    ...
    root.wm_title('SUSHANT MP3 Player')

    img = PhotoImage(file='./Images/Images.png')
    next_ = PhotoImage(file='./Images/Next1.png')
    prev = PhotoImage(file='./Images/previous1.png')
